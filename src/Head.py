...
class _FCNHead(nn.Module):
    def __init__(self, in_channels, channels, norm_layer=nn.BatchNorm2d, bn_momentum=0.003):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_channels, inter_channels, (3, 3), padding=1, bias=False),
            norm_layer(inter_channels, momentum=bn_momentum),
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),
            nn.Conv2d(inter_channels, channels, (1, 1))
        )
...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels, dilation_rates=(12, 24, 36), hidden_channels=256,
        ...
        self.map_convs = nn.ModuleList([
            ...
            nn.Conv2d(in_channels, hidden_channels, (3, 3), bias=False, dilation=dilation_rates[0],
                      padding=dilation_rates[0]),
            nn.Conv2d(in_channels, hidden_channels, (3, 3), bias=False, dilation=dilation_rates[1],
                      padding=dilation_rates[1]),
            nn.Conv2d(in_channels, hidden_channels, (3, 3), bias=False, dilation=dilation_rates[2],
                      padding=dilation_rates[2])
        ])
        self.map_bn = norm_act(hidden_channels * 4)

        self.global_pooling_conv = nn.Conv2d(in_channels, hidden_channels, (1, 1), bias=False)
        self.global_pooling_bn = norm_act(hidden_channels)

        self.red_conv = nn.Conv2d(hidden_channels * 4, out_channels, (1, 1), bias=False)
        self.pool_red_conv = nn.Conv2d(hidden_channels, out_channels, (1, 1), bias=False)
...
class Head(nn.Module):
    def __init__(self, classify_classes, norm_act=nn.BatchNorm2d, bn_momentum=0.0003):
        ...
        self.classify = nn.Conv2d(in_channels=256, out_channels=self.classify_classes, kernel_size=(1, 1),
                                  stride=(1, 1), padding=0, dilation=(1, 1), bias=True)
